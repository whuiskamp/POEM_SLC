...
def regrid_rest(path):
	...
	# Regridding is done using cdo and employs second-order conservative method. 
    ...
    if os.path.isfile(str(path)+'test_data/rsl.nc'): # Change this path later for production runs (should not link to 'test_data')
    ...
    if VILMA_couple:
    	sp.run(['cp', str(path)+'INPUT/topog.nc', str(path)+'coupling_data/topog_new.nc'])
    	master_topo = CDF(str(path)+'POEM_input/master_topo.nc','r',) # Import VILMA restart on MOM grid
    	VILMA_data  = CDF(str(path)+'coupling_data/vilma2mom.nc','r',) # Import master topog. on MOM grid
    	topo_m = master_topo.variables['depth'][:,:]; master_topo.close()  
    	slr    = VILMA_data.variables['slr'][-1,:,:]; VILMA_data.close() 
    	depth_new = topo_m - slr

    	topog_new = CDF(str(path)+'coupling_data/topog_new.nc','r+')   # Import what will be the new topogrpahy file
    	depth     = topog_new.variables['depth'][:,:]
    	depth[:,:]= depth_new
    	topog_new.close()

    else:
    	sp.run(['cp', str(path)+'INPUT/topog.nc', str(path)+'coupling_data/topog_new.nc']) # Doesn't need to be updated, just copy the file as-is.
